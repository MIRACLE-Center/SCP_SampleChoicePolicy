...
class Tester(object):
    def __init__(self, logger=None, config=None, args=None, mode=None):
        dataset_1 = HandXray(config['dataset']['pth'], config['dataset']['label_path'], 'Test')
...
def preprocessing(logger, config, args):
    ...
    ckpt = "./pretrain/hand_oldv1/model_epoch_1259.pth"
...
def review_multi(logger, config, args):
    ...
    # pred_record_pth = config['pred_record_pth'] = "/home1/quanquan/code/landmark/code/runs-ana/multi_pre/pred_record_list.npy"
    ...
    if not args.review:
        ...
        while True:
            if num_ref > 1:
                ids = np.random.choice(100, size=num_ref, replace=False)
                ids = np.sort(ids)
            else:
                ids = [count]
                if count >= 100: break
            ...
            mre = res['mre']
            if mre < best_mre['mre']:
                best_mre = {'mre': mre, "sim": conf}
            if conf > best_sim['sim']:
                best_sim = {'mre': mre, "sim": conf}
            ...
            if count % 1 == 0:
                # print(mre, conf)
                print(f"Best mre dict: ", best_mre)
                print(f"Best sim dict: ", best_sim)
