...
from utils.tester_byol import Tester
...
class Learner(LearnerWrapper):
    def __init__(self, config, logger, **kwargs):
        ...
        assert config['training']['non_local'] == True
        unet = UNet_Pretrained(n_channels=3, emb_len=config['training']['emb_len'], non_local=config['training']['non_local'])
        self.unet = unet
        self.net = BYOL(unet, image_size=config['dataset']['patch_size'], forward_func=4, projection_size=config['training']['emb_len'])
